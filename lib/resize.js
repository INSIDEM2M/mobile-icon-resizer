...
function getDimension(size,scale){
  console.log(size,scale)
  return {width: size.split("x")[0].trim() * scale.split("x")[0].trim() ,
         height: size.split("x")[1].trim() * scale.split("x")[0].trim()}
}

function executeResize(options, callback) {
  if(options.dimension)
    var dimensions = options.dimension.width + 'x' + options.dimension.height;
  else
...
function convertiOS(options, callback) {
  ...
  function handleImage(image, done) {
    var dimension = getDimension(image.size , image.scale);
